...
for fmri_sub in args.fmri_subjects:
    ...
    for h, fmri_hemi in enumerate(args.fmri_hemis):
        for eeg_time_split in range(args.tot_eeg_time_splits):
            ...
            # Load the correlation results
            file_name = (f'correlation_time_split-{eeg_time_split:02}_'
                f'fmri_sub-{fmri_sub:02}_hemi-{fmri_hemi}.npy')
...
for roi in rois:
        ...
        for h, hemi in enumerate(args.fmri_hemis):
            ...
            if h == 0:
                ...
                corr_sub = np.append(corr_sub,
                    correlation[f'hemi-{hemi}'][fs,roi_idx], 0)
    ...
    roi_correlation[roi] = np.array(corr)
    del corr
...
ci_roi_correlation = {}
ci_dist = {}
n_times = correlation['hemi-left'].shape[2]
for roi in rois:
