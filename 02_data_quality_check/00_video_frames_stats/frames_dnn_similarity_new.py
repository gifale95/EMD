...
parser.add_argument('--video', default=0, type=int) # [0, 1101]
...
print('\n\n\n>>> EEG videos - Frames DNN similarity <<<')
print('\nInput arguments:')
...
# =============================================================================
# Extract the video frames DNN features
# =============================================================================
feats = {}
feats['block1'] = []
feats['block2'] = []
...
for idx in frames_idx:
	...
	for f, feat in enumerate(x):
		feats[model.feat_list[f]].append(np.squeeze(feat.data.cpu().numpy()))
feats['block1'] = np.asarray(feats['block1'])
feats['block2'] = np.asarray(feats['block2'])
feats['block3'] = np.asarray(feats['block3'])
feats['block4'] = np.asarray(feats['block4'])
feats['fc'] = np.asarray(feats['fc'])
...
# =============================================================================
feats_corr = {}
for f, feat in feats.items():
	if f != 'fc':
		feats_corr[f] = np.ones((total_frames, total_frames, feats[f].shape[2],
			feats[f].shape[3]))
	else:
		feats_corr[f] = np.ones((total_frames, total_frames))

for f, feat in feats.items():
	if f != 'fc':
		for d1 in tqdm(range(feat.shape[2])):
			for d2 in range(feat.shape[3]):
				for f1 in range(total_frames):
					for f2 in range(f1):
						feats_corr[f][f1,f2,d1,d2] = corr(feat[f1,:,d1,d2],
							feat[f2,:,d1,d2])[0]
						feats_corr[f][f2,f1,d1,d2] = feats_corr[f][f1,f2,d1,d2]
	else:
		for f1 in range(total_frames):
			for f2 in range(f1):
				feats_corr[f][f1,f2] = corr(feat[f1], feat[f2])[0]
				feats_corr[f][f2,f1] = feats_corr[f][f1,f2]

for f, feat in feats_corr.items():
	if f != 'fc':
		feats_corr[f] = np.mean(np.mean(feat, 2), 2)


# =============================================================================
# Save the features correlations
# =============================================================================
save_dir = os.path.join(args.project_dir, 'results', 'video_frames_similarity',
	'dnn_similarity_new')
if not os.path.isdir(save_dir):
	os.makedirs(save_dir)
file_name = 'resnet50_similarity_video-' + format(args.video+1, '05')
np.save(os.path.join(save_dir, file_name), feats_corr)