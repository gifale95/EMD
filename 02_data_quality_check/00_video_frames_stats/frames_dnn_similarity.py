...
for v, video in enumerate(tqdm(videos_list)):
	...
	for idx in frames_idx:
		...
		for f, feat in enumerate(x):
			feats[model.feat_list[f]].append(feat.data.cpu().numpy().flatten())
	...
	for f, feat in feats.items():
		for v1 in range(total_frames):
			for v2 in range(v1):
				feats_corr[f][v1,v2] = corr(feat[v1], feat[v2])[0]
				feats_corr[f][v2,v1] = feats_corr[f][v1,v2]
	# Save the frames correlations
	file_name = 'resnet50_similarity_video-' + format(v+1, '05')
