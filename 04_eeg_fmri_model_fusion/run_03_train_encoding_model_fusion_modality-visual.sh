...
#SBATCH --job-name=05_eeg_fmri_model_fusion-03_train_encoding_model_fusion_modality-visual
...
declare -a model_name_all
index=0
for s in '1' ; do
	for h in 'left' 'right' ; do
		for c in 'append' 'average' ; do
...
model_name=${model_name_all[$SLURM_ARRAY_TASK_ID]}
...
# Run the job
python 03_train_encoding_model_fusion.py --fmri_subject $fmri_subject --fmri_hemi $fmri_hemi --eeg_channel_policy $eeg_channel_policy --fmri_split $fmri_split --modality 'visual' --model_name $model_name
