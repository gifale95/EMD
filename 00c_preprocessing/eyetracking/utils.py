def preprocess_eyetracking(args, session):
    ...
    for f in tqdm(range(len(eye_files))):
        ...
        sample_cols = ['timestamp', 'x_coordinate', 'y_coordinate',
            'pupil_size']
        samples = pd.read_csv(eye_files[f], sep="\t", header=None,
            names=sample_cols, compression='gzip')
        samples['timestamp'] = np.round(samples['timestamp'], 3).astype(np.float32) # Round the sample time to 3 digits (ms precision)
        samples['x_coordinate'] = samples['x_coordinate'].astype(np.float32)
        samples['y_coordinate'] = samples['y_coordinate'].astype(np.float32)
        samples['pupil_size'] = samples['pupil_size'].astype(np.float32)
        # Set negative and zero values (blinks & artefacts) to NaNs
        samples.loc[samples['x_coordinate'] <= 0, 'x_coordinate'] = np.nan
        samples.loc[samples['y_coordinate'] <= 0, 'y_coordinate'] = np.nan
        ...
        epoched_data = np.zeros((n_trials, len(channels), len(times)),
            dtype=np.float32)
        for e in stim_onset:
            ...
            for c, chan in enumerate(channels):
                epoched_data[:,c] = np.array(samples[chan])[idx_start:idx_end]
        del samples
        ...
        del epoched_data, stim_id, trial_run, trial_num
