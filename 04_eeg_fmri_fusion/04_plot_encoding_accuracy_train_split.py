	...
	for hemi in ['left', 'right']:
		for split in range(args.tot_fmri_splits):
			data_dir = os.path.join(args.project_dir, 'results',
				'eeg_fmri_fusion', 'encoding_accuracy_train_split', 'sfreq-'+
				format(args.sfreq, '04'), 'eeg_channel_policy-'+
				args.eeg_channel_policy, 'fmri_hemi-'+hemi,
...
save_dir = os.path.join(args.project_dir, 'results', 'eeg_fmri_fusion',
	'encoding_accuracy_train_split_plots', 'sfreq-'+format(args.sfreq, '04'),
	'eeg_channel_policy-'+args.eeg_channel_policy)
...
for t, time in enumerate(tqdm(times)):
	...
	file_name = os.path.join(save_dir, 'correlation_eeg_channel_policy-'+
...
for t, time in enumerate(tqdm(times)):
	...
	plt.show()
	# Save the surface plot
	file_name = os.path.join(save_dir, 'explained_variance_eeg_channel_policy-'+
		args.eeg_channel_policy+'_sub-average_time_'+format(t, '04')+'.png')
	fig.savefig(file_name, dpi=100, bbox_inches='tight', transparent=False,
		format='png')
	plt.close()
