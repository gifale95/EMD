...
for s, sub in enumerate(args.eeg_subjects):
	...
	# Extract the training data, and average across repeats
	idx_train = np.arange(1, 1001)
	train_data_sub = np.zeros((len(idx_train), len(ch_names), len(times)),
		dtype=np.float32)
	for v, video in enumerate(idx_train):
		idx = np.where(stimuli_presentation_order == video)[0]
		train_data_sub[v] = np.mean(eeg_data[idx], 0)
	...
	eeg_train_all.append(train_data_sub)
	eeg_test_all.append(test_data_sub)
	del train_data_sub, test_data_sub
...
if args.eeg_channel_policy == 'average':
	...
	eeg_train = np.mean(eeg_train_all, 0)
	...
	for s in range(len(args.eeg_subjects)):
		if s == 0:
			eeg_train = eeg_train_all[s]
			eeg_test = eeg_test_all[s]
		else:
			eeg_train = np.append(eeg_train, eeg_train_all[s], 1)
			eeg_test = np.append(eeg_test, eeg_test_all[s], 1)

del eeg_train_all, eeg_test_all


# =============================================================================
# Transform the EEG responses with PCA, at each time point
# =============================================================================
eeg_pca_train = np.zeros((eeg_train.shape), dtype=np.float32)
...
np.save(os.path.join(save_dir, file_name_train), eeg_pca_train_dict)
