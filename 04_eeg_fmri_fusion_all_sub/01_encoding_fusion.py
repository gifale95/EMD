...
parser.add_argument('--eeg_subjects', type=list, default=[1, 2, 3])
...
data_dir = os.path.join(args.emd_dir, 'derivatives', 'eeg',
    f'sub-{args.eeg_subjects[0]:02}')
file_name = f'sub-{args.eeg_subjects[0]:02}_eeg_metadata.npy'
times = np.load(os.path.join(data_dir, file_name), allow_pickle=True).item()['times']
...
for es, esub in enumerate(tqdm(args.eeg_subjects)):
    ...
    data_dir = os.path.join(args.emd_dir, 'derivatives', 'eeg',
        f'sub-{esub:02}')
    file_name = f'sub-{esub:02}_eeg_metadata.npy'
    metadata = np.load(os.path.join(data_dir, file_name), 
...
# =============================================================================
    ...
    eeg_train_sub = np.zeros((len(train_video_ids), eeg_sub.shape[1],
        eeg_sub.shape[2]), dtype=np.float32)
    for v, video in enumerate(train_video_ids):
        idx = np.where(stimulus_id_sub == video)[0]
        eeg_train_sub[v] = np.nanmean(eeg_sub[idx], 0)

    # EEG test responses
    test_video_ids = np.arange(1001, 1103)
    eeg_test_sub = np.zeros((len(test_video_ids), eeg_sub.shape[1],
        eeg_sub.shape[2]), dtype=np.float32)
    for v, video in enumerate(test_video_ids):
        idx = np.where(stimulus_id_sub == video)[0]
        eeg_test_sub[v] = np.nanmean(eeg_sub[idx], 0)
    del eeg_sub, stimulus_id_sub

    # Append the EEG across subjects along the channel dimension
    if es == 0:
...
save_dir = os.path.join(args.emd_dir, 'results',
    'eeg_fmri_encoding_fusion_all_sub', 'correlation')
os.makedirs(save_dir, exist_ok=True)

file_name = (f'time_split-{args.eeg_time_split:02}_fmri_sub-{args.fmri_subject:02}_'
